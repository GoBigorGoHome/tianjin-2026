...
[agc064_b](https://atcoder.jp/contests/agc064/tasks/agc064_b)

给你一个有 $N$ 个点和 $M$ 条边的连通无向图。对于每个 $i=1,2,\ldots,M$，第 $i$ 条边连接点 $a_i$ 和 $b_i$。每条边被涂成了红色或蓝色，边的颜色用字符串 $c$ 表示，$c_i$ 是 `R`（或 `B`）表示第 $i$ 条边是红色（或蓝色）。

给你一个长为 $N$，由 `R`、`B` 构成的字符串 $s$。  
...
但这不是充分条件，例如 ![h:25](./red_blue_graph_example.svg){.inline}。

这个例子启发我们，至少要有一条能满足两个点的边，也就是 ![h:14](./r3.svg){.inline} 或 ![h:14](./b3.svg){.inline}。若不然，一条边最多满足一个点，满足 $N$ 个点至少要 $N$ 条边，就不可能是树。
...
1. 能满足两个端点的边：![h:14](./r3.svg){.inline} 和 ![h:14](./b3.svg){.inline}。
2. 只能满足一个端点的边：![h:14](./rrb.svg){.inline} 和 ![h:14](./rbb.svg){.inline}。
3. 不能满足任何一个点的边：![h:14](./rbr.svg){.inline} 和 ![h:14](./brb.svg){.inline}。
...
![](./add_type_1_edges.svg){width=500}
...
![](./killer_example.svg){width=500px}
