...
![bg right h:200](https://www.cs.jhu.edu/~kosaraju/kosaraju.gif)


- 按 SCC 的**拓扑序**找出每个 SCC 
- 做两次 DFS
...
![bg right:40% h:260](https://www.cs.princeton.edu/~ret/ret-12.jpg)

- 做一次 DFS
- 按 SCC 的**逆拓扑序**找出每个 SCC
...
Tarjan 的 SCC 算法是以 SCC 的逆拓扑序找出每个 SCC 的，每找出一个 SCC 之后，就把其中的点标记一下，这样如果有从 $v$ 到 $w$ 的路径而 $w$ 没被标记，那么它就一定与 $v$ 在同一 SCC 里。
