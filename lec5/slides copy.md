...
设二进制划分把正整数 $n$ 分成 $K$ 个数 $p_1, \dots, p_K$。不难看出

> $K$ 是使 $1 + 2 + 2^2 + \dots + 2^{m-1} \ge n$ 正整数 $m$ 的最小值。

$1 + 2 + 2^2 + \dots + 2^{m-1} \ge n$ 即 $2^m \ge n + 1$，于是可知
...
设二进制划分把正整数 $n$ 分成 $K$ 个数 $p_1, \dots, p_K$。我们证明

> 每个 $a = 1, 2, \dots, n$ 都可表为 $p_1, \dots, p_K$ 中的几项之和。

情况一
$$ n = 1 + 2 + 2^2 + \dots + 2^{K-1} $$
...
其中 $1 \le t < 2^{K-1}$。
对于 $1 \le i \le t < 2^{K - 1}$，结论成立。
对于 $t < i \le n$，有 $i - t \le n - t$，结论也成立。
