...
考虑一个解里相邻两块石头 $i, j$，先吃石头 $i$ 后吃石头 $j$。如果改为先吃 $j$ 后吃 $i$，那么石头 $j$ 的能量少损失 $L_j S_i$ 单位，而石头 $i$ 的能量多损失**至多** $L_iS_j$ 单位。于是我们得到

<div class=proposition>

考虑一个解，设 $i, j$ 是其中**相邻**两块石头，$i$ 在 $j$ 之前。如果 $L_jS_i > L_iS_j$，那么交换 $i, j$ 将得到一个更好的解。如果 $LjS_i = L_iS_j$，那么交换 $i, j$ 得到的解不会更差。

</div>

<div class=corollary>

考虑一个**最优解**，设 $i, j$ 是其中**相邻**两块石头，$i$ 在 $j$ 之前。那么 $L_jS_i \le L_iS_j$。如果 $LjS_i = L_iS_j$，那么交换 $i, j$ 得到的仍是最优解。
...
考虑一个**最优解**，设 $i, j$ 是其中两块石头，$i$ 在 $j$ 之前。那么 $L_i/S_i \ge L_j/S_j$。如果 $L_i/S_i = L_j/S_j$，那么交换 $i, j$ 得到的仍是最优解。
</div>
...
设 $P$ 是把全部石头按 $L/S$ 值从大到小排列所得的序列。那么存在一个最优解是 $P$ 的子序列。

</div>

解法：把全部石头按 $L/S$ 值从大到小排列，然后用 01 背包求出一个最优子序列。

<div class=topic topic=子问题>

$f[i][t]$：从（排序后的）前 $i$ 个石头中选一些吃掉，花的总时间等于 $t$，获得的总能量的最大值。

$0 \le i \le N$，$0 \le t \le \sum_{i=1}^{N} S_i$
...
考虑两种情况：
- 不买商品 $i$，归结为 $f[i - 1][j]$。
- 买商品 $i$。设商品 $i$ 的颜色是 $c$。分两种情况：
    - 商品 $i$ 是买的第一个颜色为 $c$ 的商品。设上一种颜色的最后一个商品是第 $i'$ 个，归结为 $f[i'][j - P_i] + U_i + K$。
    - 商品 $i$ 不是买的第一个颜色为 $c$ 的商品。归结为 $f[i - 1][j - P_i] + U_i$。
